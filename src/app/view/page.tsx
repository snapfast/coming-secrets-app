...
import { decryptMessage, isDateUnlocked, getTimeRemaining, SecretData } from '@/lib/crypto';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 to-indigo-900 dark:from-gray-900 dark:to-purple-900 p-8 flex items-center justify-center">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 max-w-md w-full text-center border border-purple-800 dark:border-purple-700">
          ...
          <h1 className="text-2xl font-bold text-purple-900 dark:text-purple-100 mb-4">Secret Locked</h1>
          <p className="text-purple-800 dark:text-purple-300 mb-6">
            This secret will unlock on {new Date(secretData.unlockDate).toLocaleString()}
          </p>
          
          <div className="bg-purple-900/10 dark:bg-purple-900/20 rounded-lg p-4 mb-6">
            <h2 className="text-lg font-semibold text-purple-900 dark:text-purple-100 mb-2">Time Remaining:</h2>
            <div className="grid grid-cols-4 gap-2 text-center">
              <div>
                <div className="text-2xl font-bold text-purple-800">{timeRemaining.days}</div>
                <div className="text-sm text-purple-700">Days</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-purple-800">{timeRemaining.hours}</div>
                <div className="text-sm text-purple-700">Hours</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-purple-800">{timeRemaining.minutes}</div>
                <div className="text-sm text-purple-700">Minutes</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-purple-800">{timeRemaining.seconds}</div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 to-indigo-900 dark:from-gray-900 dark:to-purple-900 p-8">
      <div className="max-w-2xl mx-auto pt-16">
        <div className="text-center mb-8">
          <div className="text-purple-500 text-6xl mb-4">🎉</div>
          <h1 className="text-3xl font-bold text-white dark:text-purple-100 mb-2">Secret Unlocked!</h1>
          <p className="text-purple-200 dark:text-purple-300">Your time-locked message is now available</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 border border-purple-800 dark:border-purple-700">
          {secretData.senderEmail && (
            <div className="mb-4">
              <label className="block text-sm font-medium text-purple-900 dark:text-purple-300 mb-1">From:</label>
              <p className="text-gray-900 dark:text-white">{secretData.senderEmail}</p>
            </div>
          )}
          
          {secretData.recipientEmail && (
            <div className="mb-4">
              <label className="block text-sm font-medium text-purple-900 dark:text-purple-300 mb-1">To:</label>
              <p className="text-gray-900 dark:text-white">{secretData.recipientEmail}</p>
            </div>
          )}
          
          <div className="mb-4">
            <label className="block text-sm font-medium text-purple-900 dark:text-purple-300 mb-2">Secret Message:</label>
            <div className="bg-purple-900/10 dark:bg-purple-900/20 rounded-lg p-4">
              <p className="text-gray-900 dark:text-white whitespace-pre-wrap">{secretData.message}</p>
            </div>
          </div>
